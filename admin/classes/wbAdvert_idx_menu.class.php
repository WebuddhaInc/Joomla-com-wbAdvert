...
// ************************************************************************************************
//
// Menu Cross-Reference Class
//
// ************************************************************************************************

// ------------------------------------------------------------------------------------------------------ wbAdvert_idx_menu
class wbAdvert_idx_menu extends JTable {
  ...
  function save( $advert_id = null, $campaign_id = null, $set = null ){
    ...
    if( is_array( $set ) ){
      foreach( $set AS $id ){
        ...
        if( !count( $rows ) ){
          $db->setQuery("
            INSERT INTO #__wbadvert_idx_menu
            (`advert_id`, `campaign_id`, `menu_id`)
            VALUES
  ...
  function delete( $advert_id = null, $campaign_id = null, $set = null ){
    ...
    if( !is_null( $set ) )
      if( is_array($set) )
        $where[] = "`menu_id` IN ('". implode("','",$set) ."')";
      else
        $where[] = "`menu_id` = '". (int)$set ."'";
    if( count($where) ) {
      $db->setQuery("
        DELETE FROM `#__wbadvert_idx_menu`
  ...
  function show_ok( $advert_id = null, $campaign_id = null, $set = null ){
    ...
    if( is_array( $set ) ){
      // Check Many
      $db->setQuery("SELECT * FROM #__wbadvert_idx_menu WHERE `advert_id`='$advert_id' AND `menu_id` IN (".join(',',$set).")");
      $rows = $db->loadObjectList();
      return count( $rows ) ? true : false;
    } else if( $set ) {
      // Check One
      $db->setQuery("SELECT * FROM #__wbadvert_idx_menu WHERE `advert_id`='$advert_id' AND `menu_id`='$set'");
      $rows = $db->loadObjectList();
      return count( $rows ) ? true : false;
    } else {
      // Set to ALL
      $db->setQuery("SELECT * FROM #__wbadvert_idx_menu WHERE `advert_id`='$advert_id'");
      $rows = $db->loadObjectList();
      return count( $rows ) ? true : false;
    }
    return false;
