...
class wbAdvert_idx_group extends JTable {
    ...
    if( is_array( $set ) ){
      foreach( $set AS $id ){
        ...
        if( !count( $rows ) ){
          $db->setQuery("
            ...
            VALUES
            ('$advert_id', '$campaign_id', '$id', '$order')
            ");
          $db->query();
        }
      }
      ...
      if( count($rows) ){
        ...
        $db->setQuery("
          ...
          (`advert_id`, `campaign_id`, `group_id`, `ordering`)
          VALUES
          ('$advert_id', '$campaign_id', '". (int)$set ."', '$order')
          ");
        $db->query();
      }
    } else {
  ...
  function show_ok( $advert_id = null, $set = null ){
    ...
    if( is_array( $set ) ){
      // Check Many
      $db->setQuery("SELECT * FROM #__wbadvert_idx_group WHERE `advert_id`='$advert_id' AND `group_id` IN (".join(',',$set).")");
      $rows = $db->loadObjectList();
      return count( $rows ) ? true : false;
    } else if( $set ) {
      // Check One
      $db->setQuery("SELECT * FROM #__wbadvert_idx_group WHERE `advert_id`='$advert_id' AND `group_id`='$set'");
      $rows = $db->loadObjectList();
      return count( $rows ) ? true : false;
    } else {
      // Set to ALL
      $db->setQuery("SELECT * FROM #__wbadvert_idx_group WHERE `advert_id`='$advert_id'");
      $rows = $db->loadObjectList();
      return count( $rows ) ? true : false;
    }
    return false;
